...
update public.psv_data
set data_source_tag = '真实数据'
where data_source_tag is null or data_source_tag = '';

-- Site whitelist and unique key
do $$
begin
  if not exists (
    select 1 from pg_constraint where conname = 'psv_data_station_check'
  ) then
    alter table public.psv_data
      add constraint psv_data_station_check
...
begin
  if not exists (
    select 1 from pg_constraint where conname = 'psv_data_source_check'
  ) then
    alter table public.psv_data
      add constraint psv_data_source_check
      check (data_source_tag in ('真实数据', '模拟数据', '未标注'));
  end if;
end $$;
