...
def inject_ui_theme(palette: dict, enabled: bool = True):
    ...
    :root {{
      ...
      --ui-font-family: "Microsoft YaHei UI", "Microsoft YaHei", "PingFang SC", "Noto Sans CJK SC", "Source Han Sans SC", "SimHei", sans-serif;
    }}

    html, body, [data-testid="stAppViewContainer"], [data-testid="stSidebar"], .stApp,
    p, div, span, label, button, input, textarea, select, option,
    [data-baseweb], [data-testid="stMarkdownContainer"], [data-testid="stMetric"],
...
def _setup_cjk_font():
    ...
    for font_path in fallback_paths:
        if os.path.exists(font_path):
            ...
            mpl.rcParams["font.sans-serif"] = [fp.get_name(), "DejaVu Sans"]
            mpl.rcParams["axes.unicode_minus"] = False
            return
...
if "data_reload_token" not in st.session_state:
    st.session_state.data_reload_token = 0
if "alerts_reload_token" not in st.session_state:
    st.session_state.alerts_reload_token = 0
if "last_alert_sync_key" not in st.session_state:
    st.session_state.last_alert_sync_key = ""
if "active_view" not in st.session_state:
    st.session_state.active_view = "历史分析"


def invalidate_data_cache():
    st.session_state.data_reload_token += 1
    _load_scored_data_cached.clear()


def invalidate_alert_cache():
    st.session_state.alerts_reload_token += 1
    _load_alerts_cached.clear()
...
def _load_scored_data_cached(
    ...
    df_raw_local = load_data(station_scope=station_scope, role=role)
    df_scored_local = compute_scores(df_raw_local, enable_ai=enable_ai, contamination=contamination)
    return df_raw_local.copy(), df_scored_local.copy()
...
def _build_alert_sync_key(df_scored: pd.DataFrame, enable_ai: bool, contamination: float) -> str:
    if df_scored is None or len(df_scored) == 0:
        return "empty"
    last_updated = pd.to_datetime(df_scored.get("updated_at", pd.Series(dtype=str)), errors="coerce").max()
    last_updated_text = "" if pd.isna(last_updated) else last_updated.isoformat()
    consensus_sum = float(pd.to_numeric(df_scored.get("consensus_score", pd.Series(dtype=float)), errors="coerce").fillna(0).round(2).sum())
    high_count = int(df_scored["Risk_final"].eq("🔴 高风险").sum()) if "Risk_final" in df_scored.columns else 0
    escalate_count = int(df_scored.get("ai_escalate_flag", pd.Series(dtype=bool)).fillna(False).sum()) if "ai_escalate_flag" in df_scored.columns else 0
    return "|".join(
        [
            str(len(df_scored)),
            str(df_scored["date"].max()),
            last_updated_text,
            str(high_count),
            str(escalate_count),
            f"{consensus_sum:.2f}",
            str(int(bool(enable_ai))),
            f"{float(contamination):.2f}",
        ]
    )
...
def sync_alerts_from_scores(df_scored: pd.DataFrame):
    ...
    candidate_rows = df_scored[
        (df_scored.get("Risk_final", pd.Series(dtype=str)) == "🔴 高风险")
        | (df_scored.get("ai_escalate_flag", pd.Series(dtype=bool)).fillna(False))
    ].copy()
...
if st.session_state.last_alert_sync_key != sync_key:
    ...
    st.session_state.last_alert_sync_key = sync_key
    invalidate_alert_cache()

alerts = _load_alerts_cached(
    station_scope=STATION_SCOPE,
    role=ROLE,
    reload_token=st.session_state.alerts_reload_token,
)
...
# ================== Module Navigation ==================
NAV_ITEMS = ["历史分析", "AI预警中心", "驾驶舱", "告警闭环", "报表导出"]
